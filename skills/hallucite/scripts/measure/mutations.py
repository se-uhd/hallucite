...
MUTATIONS = [
    ...
    ("fetch: the download is installed without checking it at all", F,
     ...
     '    refusal = ""'),
    ("verifier: a backend failure is named by the method rather than the caller", V,
     "                return _Answer(DbResult(name, ERROR))",
     '                return _Answer(DbResult("backend", ERROR))'),
    ("verifier: one malformed reference fails the whole batch", V,
     "                return _Answer(DbResult(name, ERROR))\n\n        if not items:",
     "                raise\n\n        if not items:"),
    ("audit: a backend failure no longer marks the reference degraded", A,
     '        "degraded": bool(failed) and result.status != "verified",', '        "degraded": False,'),
    ("audit: a retraction is dropped from the record triage reads", A,
     '        "retraction_info": _retraction_info(result.retraction_info),',
     '        "retraction_info": None,'),
    ("dblp: the homonym suffix stays on the record's author name", D,
     '_HOMONYM_SUFFIX = re.compile(r"\\s+\\d{4}$")', '_HOMONYM_SUFFIX = re.compile(r"(?!x)x")'),
    ("verifier: a later backend overwrites the evidence an earlier one gathered", V,
     "        self.doi_info = self.doi_info or answer.doi_info",
     "        self.doi_info = answer.doi_info"),
